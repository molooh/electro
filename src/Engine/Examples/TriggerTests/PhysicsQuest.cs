...
namespace Examples.TriggerTests
{
    class PhysicsQuest
    {
        ...
        public ShaderProgram _spColor3;
        public IShaderParam _colorParam3;
        ...
        {
            var groundShape = _world.AddBoxShape(35, 0.1f, 35);
        ...
        public void StandingCube()
        {
            for (int k = 0; k < 1; k++)
            {
                var pos = new float3(25- k, 3+k, k);
                var stCube = _world.AddRigidBody(30, pos, float3.Zero, MyBoxCollider);

                stCube.Friction = 1.0f;
                stCube.Restitution = 0.7f;
                stCube.SetDrag(0.0f, 0.05f);
                _numRB++;
            }
        }
